...
describe("project manifest", () => {
  it("round-trips", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "strand-proj-"));
    await saveProject(dir, {
      name: "myapp",
      version: "0",
      strands: ["shared", "backend"],
      ports: { base: 4000, block: 100 },
    });
    const loaded = await loadProject(dir);
    expect(loaded.name).toBe("myapp");
    expect(loaded.strands).toEqual(["shared", "backend"]);
  });
  ...
  it("rejects invalid project name", async () => {
    ...
    await writeFile(
      ...
      JSON.stringify({ name: "BadName", strands: ["x"] }),
    );
