...
<!-- premo:start (generated by `premo adopt` — edits between these markers are overwritten) -->
...
- `premo dev` → `sh bin/dev.sh`
- `premo build` → `yarn build`
- `premo test` → `yarn test`
- `premo lint` → `yarn lint`
- `premo deploy` → `npm publish`
