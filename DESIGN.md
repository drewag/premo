...
> **Note:** §13 renamed the adapter's discovery unit from _targets_ to **packages** (and split run/deploy _targets_ out as a separate axis). Read §13 for the current model; the `targets` language below predates it.
...
Adapter precedence: `xcode(root) → workspaces(declared) → maven(root pom) → monorepo(discovered) → cli → node-scripts`. (`maven` outranks `monorepo` so a multi-module root pom builds through Maven's own reactor rather than being split into per-dir members; the leaf adapters — xcode, maven, cli, node-scripts — are also the child adapters a monorepo member can resolve to.)
